...
public function agregar()
{
   try {
    ...
    $query     = "SELECT * FROM familia WHERE codigo=:codigo";
    ...
    {
     $query     = "INSERT INTO familia(codigo,descripcion)VALUES(:codigo,:descripcion)";
    $statement = $conexion->prepare($query);
    $statement->bindParam(':codigo',$this->codigo);
...
function lista()
{
	...
	$modelo    = new Conexion();
	$conexion  = $modelo->get_conexion();
	$query     = "SELECT * FROM familia ORDER BY descripcion";
	$statement = $conexion->prepare($query); 
	$statement->execute();
	$result = $statement->fetchAll();
	return $result;
