...
function __construct($alm='',$numero='',$tran='',$item='',$codigo='',$descripcion='',$unidad='',$cantidad='',$precio='',$fecha='',$centro_costo='',$ot='',$tipo='')
{
   $this->alm               =  $alm; 
   $this->numero            =  $numero;
   $this->tran              =  $tran;
...
$modelo    = new Conexion();
$conexion  = $modelo->get_conexion();
$query     = "INSERT INTO movalmdet(alm,numero,tran,item,codigo,descripcion,unidad,cantidad,precio,fecha,centro_costo,ot,tipo)VALUES(:alm,:numero,:tran,:item,:codigo,:descripcion,:unidad,:cantidad,:precio,:fecha,:centro_costo,:ot,:tipo)";
    $statement = $conexion->prepare($query);
...
public function actualizar($id)
{
   try {
    $modelo    = new Conexion();
    $conexion  = $modelo->get_conexion();
     $query     = "UPDATE  movalmdet  SET tran=:tran,codigo=:codigo,descripcion=:descripcion,unidad=:unidad,cantidad=:cantidad,precio=:precio,fecha=:fecha,centro_costo=:centro_costo,ot=:ot WHERE  id=:id";
    $statement = $conexion->prepare($query);
    $statement->bindParam(':tran',$this->tran);
    $statement->bindParam(':codigo',$this->codigo);
    $statement->bindParam(':descripcion',$this->descripcion);
