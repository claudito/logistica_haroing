...
<?php if (count($comovd->lista($numero,'OS'))>0): ?>
<div class="modal-header">
<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
<h4 class="modal-title">Detalle de Orden de Servicio # <?php echo $numero; ?></h4>
</div>
<form id="detalle">
...
<div class="table-responsive-off">
  <table class="table table-bordered table-condensed">
    ...
    <tbody>
    <?php foreach ($comovd->lista($numero,'OS') as $key => $value): ?>
    <tr>
...
<?php if (count($requisc->lista_ordenes('RS','OS'))>0): ?>

<div class="input-group">
<select name="requerimiento" id="" class="form-control" required="">
<option value="">[ Seleccionar ]</option>
<?php foreach ($requisc->lista_ordenes('RS','OS') as $key => $value): ?>
